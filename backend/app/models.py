...
from sqlalchemy import Enum as SQLEnum  # For role enum
import uuid
...
user_skills = db.Table(
    ...
    db.Column(
        "skill_id", UUID(as_uuid=True), db.ForeignKey("skills.id"), primary_key=True
    ),
)
...
class User(db.Model):
    ...
    role = db.Column(
        SQLEnum("Member", "Admin", name="user_roles"), nullable=False, default="Member"
    )  # Simplified enum
...
class Rule(db.Model):  # New: For global assignment rules
    __tablename__ = "rules"
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    criteria_type = db.Column(db.String(50), nullable=False)  # e.g., 'payer', 'age'
    criteria_value = db.Column(db.String(255))  # e.g., 'Medicare', '>65'
    strategy = db.Column(db.String(50), nullable=False)  # e.g., 'seniority', 'age'
    priority = db.Column(db.Integer, nullable=False, default=1)  # Order of application
