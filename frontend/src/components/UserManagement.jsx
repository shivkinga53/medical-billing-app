...
const UserManagement = () => {
    ...
    const openEditModal = (user) => {
        ...
        setFormData({
            ...
            skill_ids: user.skills ? allSkills.filter(s => user.skills.includes(s.name)).map(s => s.id) : [],
        });
        setIsModalOpen(true);
    };
        ...
        <div>
            ...
            <table>
                ...
                <tbody>
                    {users.map(user => (
                        <tr key={user.id}>
                            <td>{user.name}</td><td>{user.username}</td><td>{user.role || 'N/A'}</td>
                            <td>{user.is_active ? 'Active' : 'Inactive'}</td>
                            <td>
                                <button onClick={() => openEditModal(user)}>Edit</button>
                                <button onClick={() => handleStatusToggle(user.id, user.is_active)}>
                        ...
                        <form onSubmit={handleSaveChanges}>
                            <div>
                                ...
                                <select name="role" value={formData.role} onChange={handleFormChange}>
                                    <option value="Sr. Biller">Sr. Biller</option>
                                    <option value="Biller">Biller</option>
                                </select>
                            </div>
                            <div>
                                <label htmlFor="max_daily_claims">Max Daily Claims: </label>
                                <input type="number" name="max_daily_claims" value={formData.max_daily_claims} onChange={handleFormChange} />
                            </div>
                            <div>
                                <label htmlFor="seniority">Seniority: </label>
                                <input type="number" name="seniority" value={formData.seniority} onChange={handleFormChange} />
                            </div>
                            
                            {/* --- NEW DROPDOWN --- */}
                            <div>
                                <label htmlFor="assign_by">Assignment Strategy (assign_by): </label>
                                {/* Admin will assign... assign_by i.e. (examples: assign by payer, by age,...) */}
                                <select name="assign_by" value={formData.assign_by} onChange={handleFormChange}>
                                    <option value="payer">Payer (Default Round-Robin)</option>
                                    <option value="age">Age (Oldest Claims First)</option>
                                    <option value="seniority">Seniority (Highest Priority Claims First)</option>
                                </select>
                            </div>
                            {/* --- END NEW DROPDOWN --- */}
